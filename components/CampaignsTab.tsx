        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
          ...
          {campaigns.map((campaign) => {
            ...
            return (
              <div
                key={campaign.id}
                className="grid grid-cols-[1.5fr_90px_90px_90px_110px_130px] items-center gap-4 border-b border-gray-50 px-4 py-3 transition-colors last:border-b-0 hover:bg-gray-50/50"
              >
                <span className="truncate text-sm font-medium text-gray-900">{campaign.name}</span>
                <span className="flex items-center gap-1.5">
                  <span className={`h-2 w-2 rounded-full ${sc.dot}`} />
                  <span className={`text-xs font-medium ${sc.text}`}>{sc.label}</span>
                </span>
                <span className="text-sm text-gray-500">{campaign.productCount}</span>
